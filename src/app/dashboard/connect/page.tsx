...
export default function ConnectSystemPage() {
  ...
  // Pre-fill from /check page bridge (sessionStorage)
  useState(() => {
    if (typeof window === "undefined") return;
    try {
      const prefill = sessionStorage.getItem("sd_prefill");
      if (prefill) {
        const { brand: b, siteId: s, apiKey: k } = JSON.parse(prefill);
        if (b) setBrand(b);
        if (s) setSiteId(s);
        if (k) setApiKey(k);
        sessionStorage.removeItem("sd_prefill");
      }
    } catch {}
  });
        ...
        <form onSubmit={handleValidate} className="space-y-6">
          {brand === "enphase" && (
            <div className="space-y-3">
              <a
                href="/api/enphase/authorize?returnTo=/dashboard"
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-xs"><span className="px-2 bg-white text-gray-400">or enter credentials manually</span></div>
              </div>
            </div>
          )}
